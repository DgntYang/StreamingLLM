...
# Parameters
RUN_NAME = "timechat-baseline-sim0_5-vstream_movienet"  # timechat-baseline-sim0_7, timechat-baseline-simfull
...
RESULT_ROOT = "outputs/eval/vstream-movienet"
...
def select_frame_paths(video_path, start_frame, end_frame=None, gt_duration=0):
    frame_paths = sorted(
    ...
    if start_frame not in basename_to_idx:
        raise ValueError(
            f"Frame range not found in {video_path}: start={start_frame}"
        )
    start_idx = basename_to_idx[start_frame]
    if end_frame in basename_to_idx:
        end_idx = basename_to_idx[end_frame]
    elif gt_duration:
        end_idx = min(start_idx + int(gt_duration) - 1, len(frame_paths) - 1)
    else:
        raise ValueError(
...
if __name__ == "__main__":
    ...
    for task in tqdm(task_jsons, desc="Inference", total=len(task_jsons)):
        try:
            with torch.no_grad():
                ...
                video_end = task.get('end_time')
                gt_duration = task.get('gt_duration', 0)
                duration = task.get('duration', 0)
                video_path = osp.join(VIDEO_DIR, video_id)
                frame_paths = select_frame_paths(video_path, video_start, video_end, gt_duration=gt_duration)
                if not frame_paths:
                ...
                for start in range(0, num_frames, INPUT_STEP):
                    ...
                    # print(f'Vision_process_time_{vision_process_time * 1000:.2f}_ms_{chunk_num_frames}_frames')
    ...
    ## TODO Continue to finish code here!!
