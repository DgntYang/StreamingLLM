...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_model_tokenizer(self):
        ...
        if hasattr(self.args, 'sim_threshold'):
            self.model.config.sim_threshold = args.sim_threshold

        if hasattr(self.args, 'vision_pool_type'):
            self.model.config.vision_config.vision_pool_type = args.vision_pool_type

        if hasattr(self.args, 'spatial_keep_ratio'):
            self.model.config.vision_config.spatial_keep_ratio = args.spatial_keep_ratio

        if hasattr(self.args, 'chunk_size'):
            self.model.config.chunk_size = args.chunk_size

        if hasattr(self.args, 'drop_method'):
            self.model.config.drop_method = args.drop_method

        if hasattr(self.args, 'nearest_window_size'):
            self.model.config.nearest_window_size = args.nearest_window_size

        if hasattr(self.args, 'attend_chunk_num'):
            self.model.config.attend_chunk_num = args.attend_chunk_num

        if hasattr(self.args, 'user_query_retrieval'):
            self.model.config.user_query_retrieval = args.user_query_retrieval

        if hasattr(self.args, 'sliding_window'):
            self.model.config.sliding_window = args.sliding_window
