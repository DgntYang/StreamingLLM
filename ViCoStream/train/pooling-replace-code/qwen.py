...
class Qwen2VLTemplate(Template):
    ...
    def add_video_interval_tokens(self, inputs, labels, loss_scale, video_grid_thw):
        pass
        ...
        for media_type in ['images', 'videos']:
            if locals()[media_type]:
                ...
                for i, idx in enumerate(idx_list):
                    ...
                    # if self.config.out_streaming:
                    flag = 1
                    if flag == 0:
                        # output streaming mode, like videollm-online
                        # 先构造替换后的 media token 序列
                        media_tokens_expanded = []
                        tokens_per_frame = media_grid_thw[i][1] * media_grid_thw[i][2] // merge_length
                        for _ in range(token_len // tokens_per_frame - 1):
                            media_tokens_expanded.extend([media_token] * tokens_per_frame)
                            media_tokens_expanded.append(self.video_interval_token)
                        media_tokens_expanded.extend([media_token] * tokens_per_frame)
                        ...
                        if labels:
                            # 对 labels 也要同步填充，逗号需要正常预测，不要 -100
                            expanded_labels = [-100 if t == media_token else t for t in media_tokens_expanded]
                            labels = (
                                    labels[:idx + added_tokens_len]
                                    + expanded_labels
                                    + labels[added_tokens_len + idx + 1:]
                            )
                        added_tokens_len += len(media_tokens_expanded) - 1
                    else:
                        input_ids = input_ids[:idx
                                               + added_tokens_len] + [media_token] * token_len + input_ids[
                                                                                                 added_tokens_len
                                                                                                 + idx + 1:]
                        if labels:
                            labels = labels[:idx + added_tokens_len] + [-100] * token_len + labels[
                                                                                            added_tokens_len + idx
                                                                                            + 1:]
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if pixel_values is None and pixel_values_videos is None:  # plain-text
            ...
            if pixel_values_videos is not None:
                ...
                # video_embeds, video_grid_thw, spatial_mask = model.visual(pixel_values_videos, grid_thw=video_grid_thw, vision_pool_type, spatial_keep_ratio)
                vision_pool_type = model.config.vision_config.vision_pool_type
                spatial_keep_ratio = model.config.vision_config.spatial_keep_ratio
                video_embeds, video_grid_thw, spatial_mask = model.visual(pixel_values_videos, grid_thw=video_grid_thw, vision_pool_type=vision_pool_type, spatial_keep_ratio=spatial_keep_ratio)
        ...
        position_ids, _ = model.get_pooling_rope_index(
            input_ids, image_grid_thw, video_grid_thw, attention_mask=inputs['attention_mask'],
            vision_pool_type=vision_pool_type, **kwargs)

        # position_ids, _ = model.get_rope_index(
        #     input_ids, image_grid_thw, video_grid_thw, attention_mask=inputs['attention_mask'], **kwargs)

        # TimeChat-Online modify
        return {'inputs_embeds': inputs_embeds, 'position_ids': position_ids.contiguous(), 'input_ids': input_ids,
                'pixel_values_videos': pixel_values_videos, 'video_grid_thw': video_grid_thw,
                'labels': inputs['labels'], 'spatial_mask': spatial_mask}
